...
rhit.DetailPageController = class {
	constructor() {
		...
		document.querySelector("#inputFile").addEventListener("change", (event) => {
			...
			rhit.fbSingleCollectionManager.uploadPhotoToStorage(file);
		});
	...
	updateView() {
		...
		for(let i = 0; i < rhit.fbSingleCollectionManager.length; i++){
			const photo = rhit.fbSingleCollectionManager.getCardAtIndex(i);
	...
	_createCard(card){
		return htmlToElement(`<div class="pin" id="shTHWfQOPIxXAnEKwCUU">
        <img
          src="${card.url}"
          alt="${card.name}">
	...
	updateAuthorBox() {
		...
		document.querySelector("#authorBox").onclick = (event) => {
			if (rhit.fbSingleCollectionManager.author == rhit.fbAuthManager.uid) {
				window.location.href = "/profile.html";
			}
		};
	}
...
rhit.FbSingleCollectionManager = class {
	constructor(collectionId) {
		this._documentSnapshot = []; //object, not array
		this._unsubscribe = null;
		this._ref = firebase.firestore().collection(collectionId);
	...
	add(set, fileUrl){
		this._ref.add({
			...
			[rhit.FB_KEY_LAST_TOUCHED]: firebase.firestore.Timestamp.now(),
			[rhit.FB_COLLECTION_CARDS]:this._collectionId
		})
	}

	uploadPhotoToStorage(file, set) {
		const metadata = {
...
rhit.FbAuthManager = class {
	...
	signIn() {
		...
		Rosefire.signIn("f5a998a0-22a2-4eec-a8ba-e7e754f57541", (err, rfUser) => {
			if (err) {
